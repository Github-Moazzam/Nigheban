...
- [◐] N1.2 — `expo-battery` **is** installed (`~57.0.2`), along with `expo-task-manager` and `expo-dev-client`. **`@notifee/react-native` is genuinely still missing** — it is the one dependency this milestone named that has not landed, and N3.3 is the thing waiting on it.
...
| N1 Config plugin + dev build | M2 | 0 | 3 h | ◐ N1.1/N1.3 done; notifee missing, N1.4 waits on D2 |
| N2 Foreground service | M2 | 2 | 8 h | ◐ N2.1/N2.2/N2.5 done; boot receiver + watchdog open |
| N3 Push + alarm | M2 | 2 | 4 h | ◐ N3.1 done and observed; N3.2 fixed §14; N3.3/N3.4 open |
